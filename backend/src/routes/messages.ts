...
function extractToken(req: any): string | null {
  ...
  if (auth && auth.startsWith('Bearer ')) return auth.split('Bearer ')[1];
...
async function ensureParticipant(conversationId: number, userId: string | undefined): Promise<void> {
  ...
  if (!parseParticipants(conv).includes(userId)) throw new Error('Forbidden: Not a conversation participant');
}

async function emitNewMessageToConversation(conversationId: number, payload: any) {
  const io = getIO();
  if (!io) return;
  const conv = await Conversation.findByPk(conversationId);
  if (!conv) return;
...
router.post('/conversations', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = String(req.userId);
    const adminId = await getAdminUserId();
    if (!adminId) return res.status(500).json({ error: 'No admin configured' });
...
router.get('/attachments/:id/download', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await Conversation.update(
      { lastMessage: data.content, lastMessageAt: new Date(), status: 'active' },
...
router.delete('/:id', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (String(message.senderId) !== req.userId) return res.status(403).json({ error: 'Forbidden' });
...
router.patch('/:conversationId/read', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const conversationId = parseInt(req.params.conversationId);
    await ensureParticipant(conversationId, req.userId);

    await Message.update(
      { isRead: true, seenAt: new Date(), deliveredAt: new Date() },
      {
        where: {
          conversationId,
