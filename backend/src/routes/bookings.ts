...
async function resolveServiceId(serviceId: number | string): Promise<number> {
...
router.get('/', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
amount: data.amount,
    ...
    console.error('Failed to create booking:', error); res.status(500).json({ error: 'Failed to create booking' });
  }
});

router.patch('/:id/status', verifyTokenMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (error instanceof z.ZodError) return res.status(400).json({ error: error.errors });
    console.error('Failed to update booking:', error); res.status(500).json({ error: 'Failed to update booking' });
  }
});
