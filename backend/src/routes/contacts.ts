...
router.post('/', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    console.error('Failed to send contact message:', error); res.status(500).json({ error: 'Failed to send message' });
  }
});
