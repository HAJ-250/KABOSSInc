...
const typingUsers = new Map<string, { userId: number; name: string; conversationId: number; expiresAt: number }>();
...
function getAdminUserId(): Promise<number | null> {
...
export function isUserOnline(userId: number | string): boolean {
...
export function initSocket(io: SocketIOServer) {
  ...
  io.use((socket: AuthSocket, next) => {
    try {
      ...
      if (!JWT_SECRET) return next(new Error('Server misconfigured'));
  ...
  io.on('connection', async (socket: AuthSocket) => {
    ...
    socket.on('message:send', async (payload: any, ack?: (resp: any) => void) => {
      try {
        ...
        await Conversation.update(
          { lastMessage: content || '📎 Attachment', lastMessageAt: new Date() },
