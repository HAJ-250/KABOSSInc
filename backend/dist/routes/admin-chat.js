...
async function emitMessageToConversation(conversationId, payload) {
    const io = getIO();
    if (!io)
        return;
...
function customerIdOf(conv, adminId) {
...
router.post('/customers/:customerId/conversation', async (req, res) => {
    try {
        const adminId = String(req.userId);
...
router.get('/conversations/:conversationId/messages', async (req, res) => {
    try {
        ...
        if (!parseParticipants(conv).includes(adminId))
...
router.post('/conversations/:conversationId/messages', async (req, res) => {
    try {
        ...
        await Conversation.update({ lastMessage: text, lastMessageAt: new Date(), status: 'active' }, { where: { id: conv.id } });
...
router.post('/conversations/:conversationId/attachments', upload.array('files', 20), async (req, res) => {
    try {
        const adminId = String(req.userId);
        const conv = await Conversation.findByPk(req.params.conversationId);
        if (!conv)
        ...
        await Conversation.update({ lastMessage: message.content, lastMessageAt: new Date(), status: 'active' }, { where: { id: conv.id } });
