...
import { generateTransactionReference, normalizePhoneNumber, isValidMoMoNumber, requestToPay, getTransactionStatus, mapStatusToPaymentStatus, isSandbox, } from '../lib/momoService.js';
...
router.post('/confirm', async (req, res) => {
    try {
        ...
        if (status.financialTransactionId)
            payment.financialTransactionId = status.financialTransactionId;
        if (status.reason)
            payment.failureReason = status.reason;
        await payment.save();
        if (newStatus === 'SUCCESS') {
            ...
            await booking.save();
            // Notify customer
            await notifyUser(booking.userId, 'payment', 'Payment successful', 'Your payment was successful. Your KABOSS Inc booking has been confirmed.');
            // Notify admins
            const admins = await User.findAll({ where: { role: 'admin' }, attributes: ['id'] });
            await Promise.all(admins.map((a) => notifyUser(Number(a.getDataValue('id')), 'payment', 'New payment received', `New payment received from customer for "${booking.serviceName}" (${payment.amount} RWF).`)));
        }
...
router.post('/callback', async (req, res) => {
    try {
        const body = req.body || {};
        const momoReferenceId = body?.referenceId || body?.externalId || body?.transaction?.referenceId || '';
        if (!momoReferenceId) {
