    ...
    try {
        ...
        const conversations = all.filter((c) => {
            if (!c.participants)
                return false;
            try {
                return JSON.parse(c.participants).includes(req.userId);
            }
            catch {
                return false;
            }
...
router.post('/', verifyTokenMiddleware, async (req, res) => {
    try {
        ...
        await Conversation.update({ lastMessage: data.content, lastMessageAt: new Date() }, { where: { id: data.conversationId } });
