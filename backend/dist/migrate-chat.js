...
async function run() {
    ...
    const msgCols = await sequelize.query(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'Messages'`, { type: QueryTypes.SELECT });
    ...
    const tables = await sequelize.query(`SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'Attachments'`, { type: QueryTypes.SELECT });
    if (tables.length === 0) {
...
//# sourceMappingURL=migrate-chat.js.map